...
lab:
  ...
  description: Use a network security groups to filter inbound and outbound network traffic to and from Azure resources in an Azure virtual network. Network security groups contain security rules that filter network traffic by IP address, port, and protocol.
  duration: 20 minutes
  level: 200
  islab: true
  primarytopics:
